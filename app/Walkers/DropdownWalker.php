...
class DropdownWalker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if ($depth === 0 && $has_children) {
            ...
            $output .= '<a href="' . esc_attr($item->url) . '" class="inline-flex items-center gap-x-1 text-lg font-medium text-white hover:text-gray-300">';
        ...
        // Case 2: Pozostałe elementy (zwykłe linki na górze lub linki wewnątrz dropdownu)
            ...
            $link_classes = '';
            if ($depth > 0) {
                $link_classes = 'block rounded-xl px-6 py-4 text-sm text-white hover:bg-primary hover:text-white';
            } else {
                $link_classes = 'text-lg font-medium text-white hover:text-gray-300';
            }
